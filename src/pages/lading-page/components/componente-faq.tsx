...
export function ComponenteFAQ(props: TypeComponenteFAQ) {
  ...
  return (
    <div data-aos={props.direcao ? 'fade-left' : 'fade-right'}>
      <article
        className={`relative w-[500px] max-md:w-full ${isOpen ? '' : 'border-primary-100 rounded-2xl border-b-2'} flex h-auto flex-col items-start justify-center gap-2 px-4 py-2 transition-all duration-500 ease-in-out`}
      >
        {/* container pergunta  */}
        ...
        <div
          className={` ${isOpen ? 'mt-1 max-h-[500px] opacity-100' : 'mt-0 max-h-0 opacity-0'} text-primary-800 w-full transition-all duration-1000 ease-in-out`}
        >
          <p>{props.resposta}</p>
        </div>
      </article>
