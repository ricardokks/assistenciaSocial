...
import { matchInstituicao } from '../../../utils/buscarInstituicaoFormatada'
...
export function Instituicoes(data: IHomeProps) {
  ...
  const [abrirModalCreate, setAbrirModalCreate] = useState(false)
  ...
  const instituicoesFiltradas = instituicoes.filter((inst) =>
    matchInstituicao(inst.unidade, search)
  )
      ...
      <div className="max-lg:pb-30 flex size-full flex-col">
        ...
        <div className="mt-3 flex w-full justify-between gap-4">
          <div className="relative w-2/3">
            <IconeSearch className="absolute mt-3 translate-x-3" />
            <input
          ...
          <button
            className="bg-primary-800 max-md:text-sm font-outfit-bold hover:bg-primary-800/90 flex items-center justify-center gap-3 rounded-[5.97px] p-2 text-white duration-500 ease-in-out"
        ...
        <div className="bg-primary-800/20 mt-4 h-[2px] w-full" />
