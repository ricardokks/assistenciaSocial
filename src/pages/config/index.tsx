    ...
    <main className="font-outfit flex min-h-screen w-full flex-col items-center bg-[#EAEAEA] ">
      ...
      <header className="bg-primary-800 flex w-full max-w-[150rem] items-center gap-3 px-6 py-4 text-white">
        <button className="transition-opacity hover:opacity-80">
          <ChevronLeft className="size-8" onClick={() => navigate(-1)} />
        </button>
        ...
        <div className="flex flex-col space-y-5 rounded-2xl bg-white p-6 shadow-sm">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500">Aumente ou diminua o tamanho das fontes</p>
            </div>

            <div className="w-1/9 relative mt-2 min-w-20">
              <select
                {...register('tamanhoFonte', { valueAsNumber: true })}
                className="w-full appearance-none rounded-lg border border-gray-300 bg-white p-2 text-sm text-black outline-none "
              ...
              <ChevronDown
                className={`absolute right-1.5 top-2.5 size-5 text-black transition-all duration-500 ${
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Ative o tradutor de Libras no app</p>
              </div>
            </div>
            ...
            <button
              ...
              className={`relative h-6 w-12 rounded-full transition-colors duration-300 ${
                config?.idioma === 'LIBRAS' ? 'bg-green-500' : 'bg-gray-300'
              }`}
              onClick={() => {
            ...
            >
              <span
                className={`absolute left-0.5 top-0.5 size-5 rounded-full bg-white transition-transform duration-300 ${
                  config?.idioma === 'LIBRAS' ? 'translate-x-6' : 'translate-x-0'
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Ative ou desative as notificações</p>
              </div>
            </div>
            ...
            <button
              type="button"
              className={`relative h-6 w-12 rounded-full transition-colors duration-300 ${
                config?.notificacao ? 'bg-green-500' : 'bg-gray-300'
              }`}
              onClick={() => {
            ...
            >
              <span
                className={`absolute left-0.5 top-0.5 size-5 rounded-full bg-white transition-transform duration-300 ${
                  config?.notificacao ? 'translate-x-6' : 'translate-x-0'
                }`}
              />
            </button>
          </div>
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              className={`relative h-6 w-12 rounded-full transition-colors duration-300 ${
                config?.temaEscuro ? 'bg-green-500' : 'bg-gray-300'
              }`}
              onClick={() => {
                setAutoTheme(false)
