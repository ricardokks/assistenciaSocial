...
export function Login() {
  ...
  return (
    <div className="flex h-screen w-screen items-center justify-between overflow-hidden bg-white max-lg:flex-col-reverse">
      {/* Tela principal */}
      <div className='top-51 absolute z-10 hidden h-32 w-full max-lg:flex'> <BarsLoginMobile/> </div>
      <div className="mt-6 flex h-[90%] w-[40%] flex-col items-center py-4 max-xl:w-1/2 max-lg:h-[60%] max-lg:w-full lg:space-y-10 ">
        ...
        <div className='flex w-full flex-col items-center justify-center space-y-5 max-lg:space-y-1'>
          ...
          <div className='mt-6 flex w-3/4 flex-col items-center -space-y-2 max-lg:mt-2'>
            <h1 className=' text-primary-800 font-outfit-bold text-3xl max-xl:text-2xl'>SEJA BEM-VINDO(A)</h1>
            <h2 className='text-primary-800 font-satoshi  text-center text-[25px] font-medium max-xl:text-xl'>Assistência social na palma da mão</h2>
          </div>
        </div>
        {/* formulário */}
        <form className='flex w-full flex-col items-center justify-center space-y-3' onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="w-2/3 flex-col items-center rounded-2xl p-2 max-lg:w-5/6">
              ...
              <div className='relative flex'>
                ...
                <button
                  ...
                  {visiblePassword ? <IconEyeClose className='text-primary-800 size-6' /> : <IconEyeOpen className='text-primary-800 size-6' />}
                </button>
              </div>
            </div>
          </div>
          ...
          {/* esqueci a senha */}
          <h1 className='font-outfit text-primary-800 text-center'>Esqueceu sua senha? <a className='font-outfit-bold cursor-pointer'>Entre em contato com um <br />administrador</a></h1>
        </form>
      </div>
      <div className="relative h-full w-3/5 max-xl:w-1/2 max-lg:hidden" style={{ backgroundImage: `url(${imagemMassape})`, backgroundSize: "cover", }} />
      <div className="absolute top-0 z-0 flex h-72 w-full items-center justify-center lg:hidden" style={{ backgroundImage: `url(${imagemMassapeMobile})`, backgroundSize: "cover", }} > <img alt="" className='min-w-72 max-w-72' src={logomassape} /></div>
    </div>
  )
}