...
export function CardProjeto(props: any) {
  ...
  return (
    <div data-aos={props.animation ? 'fade-right' : ''}>
      <article className="font-outfit animate-scale-in flex flex-col items-start justify-between gap-4 rounded-2xl bg-white p-6 transition-all duration-700 ease-in-out min-h-[280px] max-h-[290px] max-md:min-w-[500px] max-md:max-w-[500px]">
        {/* container informações sobre projeto  */}
        <div className="flex items-center justify-center gap-4">
          {/* container foto do projeto */}
          <div>
            <img alt={`Foto do projeto: ${props.unidade}`} src={props.foto === null ? fotoPadraoInstituicao : props.foto} width={100} />
          </div>
