...
export function AgendamentoCard({
  ...
  // Observa posição do drag
  useEffect(() => {
    const unsubscribe = x.onChange((value) => {
      setIsEyeOpen(value > 0)
      setIsTrashOpen(value < 0)
    })
    return () => unsubscribe()
  }, [x])

  // Rotação e escala do card (só pra dar leve movimento)
  const rotate = useTransform(x, [-150, 0, 150], [-4, 0, 4])
  const scaleCard = useTransform(x, [-150, 0, 150], [0.98, 1, 0.98])

  // Apenas sombra dinâmica
  const shadow = useTransform(
    x,
    [-150, 0, 150],
    [
      '0px 0px 25px rgba(220,38,38,0.4)', // esquerda
      '0px 5px 15px rgba(0,0,0,0.1)',    // centro
      '0px 0px 25px rgba(25,74,153,0.4)', // direita
    ]
  )
  ...
  return (
    <div data-aos={animation ? 'fade-right' : ''} className="relative overflow-visible animate-scale-in ">
      ...
      <motion.div
        ...
        animate={{ rotate: isTrashOpen ? [0, -10, 10, 0] : 0 }}
        transition={{ duration: 0.5, repeat: isTrashOpen ? Infinity : 0 }}
        className="absolute right-10 top-1/2 -translate-y-1/2 z-30 flex flex-col items-center justify-center space-y-2 md:hidden"
      >
        <div className="rounded-full bg-red-100/80 p-4 backdrop-blur-sm shadow-lg ring-2 ring-red-200/50 flex items-center justify-center">
      ...
      <motion.div
        ...
        animate={{ rotate: isEyeOpen ? [0, -5, 5, 0] : 0 }}
        transition={{ duration: 0.5, repeat: isEyeOpen ? Infinity : 0 }}
        className="absolute left-5 top-1/2 -translate-y-1/2 z-30 flex flex-col items-center justify-center space-y-2 md:hidden"
      >
        <div className="rounded-full bg-blue-100/80 p-4 backdrop-blur-sm shadow-lg ring-2 ring-blue-200/50 flex items-center justify-center">
      ...
      <motion.div
        drag="x"
        style={{ x, rotate, scale: scaleCard, boxShadow: shadow }}
        dragElastic={0.15}
        ...
        <div className="mt-5 flex w-full">
          <div className="flex w-full flex-col">
            ...
            <span className="font-satoshi text-primary-800 text-[12px] font-medium">
              {item.assistencia?.servicos?.find((svc: any) => svc.id === item.servicoId)?.nome}
            </span>
          </div>

          <div className="flex w-full flex-col">
            <span className="font-outfit text-primary-800 text-[14px]">Status do agendamento</span>
